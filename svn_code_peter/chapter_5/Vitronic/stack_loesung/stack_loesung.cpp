// stack_loesung.cpp : Diese Datei enthält die Funktion "main". Hier beginnt und endet die Ausführung des Programms.
//
...
namespace vit {
    ...
    public:
        ...
        stack(stack&& that)
            : cap{that.cap}, pos{that.pos}, data{that.data}
        { 
            clear(std::move(that)); 
        }
...
int main()
{
    ...
    for (unsigned i = 0; i < si.capacity(); ++i)
        si.push(i);
